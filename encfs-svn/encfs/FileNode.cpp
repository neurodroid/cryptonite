...
ssize_t FileNode::read( loff_t offset, unsigned char *data, ssize_t size ) const
{
    IORequest req;
    req.offset = offset;
    req.dataLen = size;
...
bool FileNode::write(loff_t offset, unsigned char *data, ssize_t size)
{
...
int FileNode::truncate( loff_t size )
{
    Lock _lock( mutex );
